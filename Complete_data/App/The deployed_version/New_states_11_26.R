...
#all_states <- toupper(unique(c(corn_states, soybeans_states, wheat_states, potatoes_states)))
...
load("corn_county_distances.rda")

load("soybeans_county_distances.rda")

load("wheat_county_distances.rda")

load("potatoes_county_distances.rda")

corn_county_distances <- corn_county_distances %>%
  mutate(
    from_county = str_replace(from_county, ",\\s*([A-Z]+)$", function(m) {
      paste0(", ", str_to_title(tolower(gsub(",\\s*", "", m))))
    }),
    to_county = str_replace(to_county, ",\\s*([A-Z]+)$", function(m) {
      paste0(", ", str_to_title(tolower(gsub(",\\s*", "", m))))
    })
  )

soybeans_county_distances <- soybeans_county_distances %>%
  mutate(
    from_county = str_replace(from_county, ",\\s*([A-Z]+)$", function(m) {
      paste0(", ", str_to_title(tolower(gsub(",\\s*", "", m))))
    }),
...
potatoes_county_distances <- potatoes_county_distances %>%
  mutate(
    from_county = str_replace(from_county, ",\\s*([A-Z]+)$", function(m) {
      paste0(", ", str_to_title(tolower(gsub(",\\s*", "", m))))
    }),
...
wheat_county_distances <- wheat_county_distances %>%
  mutate(
    from_county = str_replace(from_county, ",\\s*([A-Z]+)$", function(m) {
      paste0(", ", str_to_title(tolower(gsub(",\\s*", "", m))))
    }),
...
# Create cumulative columns for each condition
df2 <- soybeans_survey_week_final %>% 
  ungroup() %>% 
  dplyr::select(
    REFERENCE_PERIOD_DESC, 
    STATE_NAME, 
...
t2 = grep("PROGRESS", names(soybeans_survey_week_final))
t1 = which(names(soybeans_survey_week_final) %in% c("STATE_NAME",
                                                "REFERENCE_PERIOD_DESC",
                                                "Year", 
                                                "Week_Num"))

df3 <- soybeans_survey_week_final[, c(t1,t2)]
df3 <- df3 %>% 
  ungroup() %>% 
  filter(STATE_NAME == "INDIANA", 
         Year == 2020) %>% 
...
# Create cumulative columns for each condition
df2 <- potatoes_survey_week_final %>% 
  ungroup() %>% 
  dplyr::select(
    REFERENCE_PERIOD_DESC, 
    STATE_NAME, 
...
t2 = grep("PROGRESS", names(potatoes_survey_week_final))
t1 = which(names(potatoes_survey_week_final) %in% c("STATE_NAME",
                                                    "REFERENCE_PERIOD_DESC",
                                                    "Year", 
                                                    "Week_Num"))

df3 <- potatoes_survey_week_final[, c(t1,t2)]
df3 <- df3 %>% 
  ungroup() %>% 
  filter(STATE_NAME == "IDAHO", 
         Year == 2020) %>% 
...
wheat_survey_week <- wheat_survey[inx,] %>% 
  filter(CLASS_DESC == "WINTER",#"ALL CLASSES"#,
         DOMAIN_DESC ==  "TOTAL"
         ) %>%
  arrange(WEEK_ENDING) %>% 
  filter(YEAR>=2000,
...
t2 = grep("PROGRESS", names(wheat_survey_week_final))
t1 = which(names(wheat_survey_week_final) %in% c("STATE_NAME",
                                                    "REFERENCE_PERIOD_DESC",
                                                    "Year", 
                                                    "Week_Num"))

df3 <- wheat_survey_week_final[, c(t1,t2)]
df3 <- df3 %>% 
  ungroup() %>% 
  filter(STATE_NAME == "MONTANA", 
         Year == 2020) %>% 
...
df3_long <- df3 %>%
  pivot_longer(
    cols = starts_with("PROGRESS"), # Select columns starting with "PROGRESS"
    names_to = "Category",
    values_to = "Value"
  ) %>%
  # Extract the type (5 YEAR AVG, PREVIOUS YEAR, or Current Year) and clean Category names
  mutate(
    Type = case_when(
      grepl("5 YEAR AVG", Category) ~ "5 Year Avg",
      grepl("PREVIOUS YEAR", Category) ~ "Previous Year",
    ...
    # Extract the progress category (e.g., PLANTED, EMERGED) by removing redundant parts
    Category = gsub("PROGRESS(, 5 YEAR AVG|, PREVIOUS YEAR)?_PCT ", "", Category)
  ) %>%
...
ggplot(data = df3_long, aes(x = Week_Num, y = Value, color = Category, linetype = Type)) +
  ...
  theme_minimal() +
  scale_color_manual(values = c("PLANTED" = "green", "HARVESTED" = "pink", "EMERGED" = "red", "HEADED" = "black")) +
  scale_linetype_manual(values = c("Current Year" = "solid", "Previous Year" = "dashed", "5 Year Avg" = "dotted")) +
  theme(legend.position = "bottom")
